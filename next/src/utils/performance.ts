    ...
    if ('PerformanceObserver' in window) {
      try {
        const lcpObserver = new PerformanceObserver((entryList) => {
          ...
          const lcpValue = Math.round(lastEntry.renderTime ?? lastEntry.loadTime ?? 0);
      ...
      try {
        const fidObserver = new PerformanceObserver((entryList) => {
          ...
          entries.forEach((entry) => {
            const timedEntry = entry as PerformanceEntry & { processingDuration?: number };
            const processingDuration = timedEntry.processingDuration ?? 0;
            const metric: VitalsMetric = {
              name: 'FID',
              value: Math.round(processingDuration),
              rating: getRating(processingDuration, thresholds.FID),
              delta: 0,
