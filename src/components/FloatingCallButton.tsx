...
export function FloatingCallButton() {
  ...
  return (
    ...
    >
      <div className="relative w-6 h-6 md:w-9 md:h-9 rounded-full text-white flex items-center justify-center shadow-xl transition-transform duration-300 hover:scale-105 animate-sway group-hover:scale-105 bg-red-600/70 md:bg-red-600">
        <Phone className="relative z-10 w-2 h-2 md:w-3 md:h-3" />
        ...
        <span className="absolute -inset-2 rounded-full bg-red-600/16 opacity-30 animate-ping transition-opacity duration-300 group-hover:opacity-60" />
      </div>

      {/* Phone number pill - visible on all sizes, responsive padding/text */}
      <span className="inline-flex items-center px-1.5 py-0.5 rounded-lg bg-red-700/80 text-white text-xs font-semibold shadow-lg select-none md:px-2 md:py-0.5 md:text-xs animate-sway-float transition-transform duration-300 ease-out transform group-hover:-translate-y-1 group-hover:scale-105 md:group-hover:bg-red-800 max-w-[9rem] truncate">
        {businessInfo.phone}
      </span>
    </a>
  );
