...
function TestimonialCard({ testimonial, index }: { testimonial: typeof testimonials[0]; index: number }) {
  ...
  return (
    ...
    >
      ...
      {/* Animated background */}
      <div className="absolute -top-10 -right-10 w-40 h-40 bg-brand-gold-soft/30 dark:bg-brand-gold/15 rounded-full blur-3xl opacity-0 group-hover:opacity-100 transition-all duration-700 group-hover:scale-150"></div>
      
      {/* Quote icon with centered alignment */}
      <div className="w-16 h-16 bg-brand-gold-soft rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 shadow-lg mx-auto">
    ...
    <section id="testimonials" className="py-32 px-6 bg-secondary dark:bg-secondary relative overflow-hidden transition-colors duration-500">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center mb-20 animate-in fade-in slide-in-from-bottom-5 duration-700">
          <span className="inline-block px-6 py-2 bg-brand-gold-soft/25 dark:bg-brand-gold/15 backdrop-blur-sm border border-brand-gold-muted/40 dark:border-brand-gold/40 rounded-full text-brand-gold-muted dark:text-brand-gold-soft text-sm mb-6 transition-colors duration-500">
            ⭐ Testimonials
          </span>
